...
function SidebarSystemActions({
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'ultron update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "ultron update";
    return (
      t.status.updateHermesConfirmMessage ??
      `This will run 'ultron update' (${cmd}) and restart the gateway when it finishes.`
    );
