      ...
      <ConfirmDialog
        ...
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'ultron update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'ultron update' (${updateInfo?.update_command ?? "ultron update"}) and restart the gateway when it finishes.`
        }
