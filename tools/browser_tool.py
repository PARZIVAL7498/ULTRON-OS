...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``ultron doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    # node_modules binary, which disappears on the next ``ultron update`` and
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    ...
    ``ultron update`` (or ``ultron doctor --fix``) warms npx's own cache
    ...
    npm code on every ``ultron update`` (the GHSA-m4m8-xjp4-5rmm class of
    ...
    cmd = [
        ...
        "--ignore-scripts",
        # --prefer-offline: once cached, repeat `ultron update`/`doctor
