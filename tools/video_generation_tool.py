...
enable video_gen/<name>``) and selecting it in ``ultron tools`` → Video
Generation.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `ultron tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `ultron tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `ultron tools` → "
...
# `ultron tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`ultron tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `ultron tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
