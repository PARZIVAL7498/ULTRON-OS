...
remediation hint pointing at ``ultron tools`` or the manual pip command.
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # presence, the `ultron update` lazy-refresh pass re-asserts THIS pin on
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``ultron update`` lazy-refresh pass.
    """
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``ultron update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def active_features() -> list[str]:
    ...
    Used by ``ultron update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``ultron update``. Never raises; lazy-install failures
