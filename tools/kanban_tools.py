...
orchestrator work. A normal ``ultron chat`` session still sees **zero**
...
(``ultron dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    Humans running ``ultron chat`` without the kanban toolset see zero
