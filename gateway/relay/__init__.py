...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``ultron gateway enroll`` writes them to
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``ultron gateway enroll``).
...
def _resolve_relay_identity_token() -> str:
    ...
    ``ultron gateway enroll`` CLI. Three modes, in precedence order:
...
def self_provision_relay() -> bool:
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``ultron gateway enroll``: has a PINNED
