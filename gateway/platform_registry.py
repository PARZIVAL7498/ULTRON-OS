...
class PlatformEntry:
    ...
    # status displays (``ultron setup``, ``ultron status``, the dashboard
    ...
    # Env vars this platform needs (for ``ultron setup`` display).
    ...
    # built-ins).  Used by ``ultron gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # invocation -- including plain `ultron chat`, which never touches any
        ...
        # `ultron setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``ultron setup``/``gateway status``, channel
