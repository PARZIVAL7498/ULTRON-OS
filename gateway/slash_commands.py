...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: ultron gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`ultron gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_heartbeat_command(self, event: "MessageEvent") -> str:
        ...
        return (
            ...
            "elapsed. Lives while the gateway runs — use `ultron cron` for durable schedules."
        )
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``ultron debug share`` from the CLI.
        """
        import asyncio
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        Spawns ``ultron update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``ultron update`` may trigger. Marker
        ...
        # Spawn `ultron update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `ultron update --gateway`
