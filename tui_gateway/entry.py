...
# ``ultron --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``ultron --tui`` path spawns ITS thread here (``_mcp_discovery_thread``),
    while the desktop app + dashboard WebSocket sidecar (``tui_gateway/ws.py``)
    and ``ultron dashboard`` spawn theirs via
