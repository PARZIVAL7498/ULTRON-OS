...
Handles: ultron gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# when hermes_constants is cached from a pre-update version during `ultron update`.
...
# — restart may fail", and `ultron update` can deadlock behind it.  The loop
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``ultron gateway status`` from falsely counting the ``hermes`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``ultron gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    try:
        if is_windows():
            ...
            # wedges the caller forever. ``ultron update`` hung exactly there
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``ultron update``
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``ultron update``.
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``ultron update`` users
    # with no running gateway until they re-invoke ``ultron gateway``
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}ultron gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}ultron gateway restart{scope_flag}"
    )
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: ultron gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `ultron gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``ultron gateway status`` output so users with
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    if is_windows():
        try:
            ...
            # dormant on every standard `ultron gateway install` deployment.
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``ultron gateway run`` must remain interruptible from
...
SERVICE_DESCRIPTION = "Ultron Gateway - Messaging Platform Integration"
...
def print_legacy_unit_warning() -> None:
    ...
    print_info("    ultron gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: ultron gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo ultron gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    ultron gateway uninstall")
    print_info("    sudo ultron gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: ultron gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    ...
    The zero-decision service path used by ``ultron setup`` (end of wizard)
    ...
    ``ultron gateway install --system`` explicitly (that path prompts and
    ...
    if is_container():
        ...
        print_info("   ultron gateway run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: ultron gateway")
        return False
    ...
    try:
        ...
        print_info("  You can retry manually: ultron gateway install")
...
def _stable_service_working_dir() -> str:
    ...
    dir, or a clone that ``ultron update`` later relocates/removes — the path
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    #
    # Under ``sudo ultron gateway … --system``, HERMES_HOME is often stripped
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``ultron gateway restart``
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo ultron gateway uninstall --system")
    print_info("         ultron gateway install")
    print_info("         ultron gateway start")
...
def systemd_install(
    ...
    # ``sudo ultron gateway install --system --force`` would bake /root/.hermes
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}ultron gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}ultron gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}ultron gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `ultron gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `ultron gateway status` or logs for final state."
            )
            return
        _wait_for_systemd_service_restart(system=system, previous_pid=pid)
        return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}ultron gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}ultron gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}ultron gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}ultron gateway start{scope_flag}"
        )
...
# process invocation (each ``ultron gateway start/stop/status`` call).
...
#      background process (the `nohup ultron gateway run` workaround). See #23387.
...
def _timestamped_stderr_gateway_command(
    ...
    *,
    ...
    ``ultron update`` sees the flag on the live grandchild argv and hands
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup ultron gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup ultron gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    # without this block every plist rewrite (e.g. `ultron gateway start`)
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  ultron gateway status             # Check status")
    from hermes_constants import display_hermes_home as _dhh
...
def launchd_stop():
    ...
    # `ultron gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # full drain budget only stalls the restart (and `ultron update`
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        ...
        print("  Run: ultron gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: ultron gateway stop")
            else:
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``ultron gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``ultron gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``ultron gateway run`` can burn memory/CPU just to fail with "already
    ...
    print("  Use 'ultron gateway restart' to replace it,")
    print("  or 'ultron gateway stop' first.")
    print("  Or use 'ultron gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `ultron gateway run` still
    ...
    # /restart) rather than through `ultron gateway restart` which already
    ...
    # `ultron gateway start/restart` — leaving the gateway vulnerable to
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``ultron gateway run`` enters through this CLI wrapper, not through
...
def _all_platforms() -> list[dict]:
    ...
    ``ultron setup gateway`` without needing the gateway to be running.
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Skipped — configure later with 'ultron gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `ultron gateway setup`.")
        return
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: ultron gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: ultron gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: ultron gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo ultron gateway install --system"
                        )
                    print_info("  Or run in foreground:  ultron gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: ultron gateway run")
                print_info(
                    "  For persistence:   tmux new -s hermes 'ultron gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: ultron gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup ultron gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'ultron gateway setup' when ready.")
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``ultron gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``ultron gateway install|uninstall|start|stop|restart``
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: ultron gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: ultron gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s hermes 'ultron gateway run'"
                )
                print()
            ...
            print()
            print(
                "  ultron gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s hermes 'ultron gateway run'         # persistent via tmux"
            )
            print(
                "  nohup ultron gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  ultron profile create <name>     # creates the s6 service slot")
                print("  hermes -p <name> gateway start   # bring it up via s6")
                print("  ultron status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: ultron gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: ultron gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: ultron gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  ultron profile delete <name>     # tears down the s6 service slot")
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: ultron gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "Use `ultron gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `ultron gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: ultron gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  ultron gateway install")
                else:
                    print("To install as a service:")
                    print("  ultron gateway install")
                    print("  sudo ultron gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  ultron gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup ultron gateway run > ~/.hermes/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s hermes 'ultron gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  ultron gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  ultron gateway install  # Install as user service")
                    print(
                        "  sudo ultron gateway install --system  # Install as boot-time system service"
                    )
