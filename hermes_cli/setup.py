...
# display_hermes_home imported lazily at call sites (stale-module safety during ultron update)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Ultron Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    ...
    print_info("  ultron config set model.provider custom")
    print_info("  ultron config set model.base_url http://localhost:8080/v1")
    print_info("  ultron config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'ultron setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'ultron setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if not _provider_ready:
        ...
        print_info("    ultron model            (pick any provider/model)")
        print_info("    ultron setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'ultron setup' to configure"))
    ...
    # Video generation — opt-in via `ultron tools` → Video Generation.
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'ultron setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'ultron setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'ultron tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'ultron setup terminal'"))
    ...
    # Spotify (OAuth via ultron auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'ultron setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('ultron setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('ultron setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('ultron setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('ultron setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('ultron setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('ultron config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('ultron config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('ultron config set <key> <value>', Colors.GREEN)}")
    ...
    print(f"   {color('ultron gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('ultron doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `ultron tools` and `ultron setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``ultron model``)
    ...
    provider added to ``ultron model`` is automatically available here.
    ...
    # Delegate to the shared ultron model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: ultron model")
    ...
    # on demand via `ultron auth add`, `ultron setup` vision, and
    # `ultron setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``ultron model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via ultron setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'ultron setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # configurable later via `ultron setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `ultron setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `ultron setup agent` later to customize.")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Open config in your editor:  ultron config edit")
    print_info("   Open config in your editor:  ultron config edit")
...
def setup_gateway(config: dict):
    ...
    if not selected:
        print_info("No platforms selected. Run 'ultron setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     ultron config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
    ...
    # via `hermes import` or `ultron setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `ultron setup tools` and `ultron tools` use the same flow:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    print_info("  Configure a model with `ultron model`.")
    print_info("  Configure tools with `ultron tools`.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      ultron setup           — full or quick (auto-detected)
      ultron setup model     — just model/provider
      ultron setup tts       — just text-to-speech
      ultron setup terminal  — just terminal backend
      ultron setup gateway   — just messaging platforms
      ultron setup tools     — just tool configuration
      ultron setup telemetry — just local shared metrics
      ultron setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Ultron Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Ultron Setup Wizard                      │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Ultron installation.             │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_info("")
        print_info("Tip: jump straight to a section with 'ultron setup model|terminal|")
    ...
    print()
    print_info("You can edit these files directly or use 'ultron config edit'")
    ...
    # Tune later with `ultron setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``ultron setup <section>``
    or switch providers with ``ultron model``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'ultron setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    ultron setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  ultron setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``ultron tools`` (which
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``ultron setup agent`` /
    ``ultron config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, hermes_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `ultron update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        ultron tools")
        ...
        print_info("  Tune agent settings: ultron setup agent")
        print()
        _print_setup_summary(config, hermes_home)
        return
...
def _blank_slate_walkthrough(config: dict, hermes_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`ultron update` runs from re-injecting them. Opt back in any")
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `ultron tools`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   ultron tools")
    print_info("  Seed skills:         hermes skills opt-in --sync")
    print_info("  Add MCP servers:     hermes mcp add")
    print_info("  Tune agent settings: ultron setup agent")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'ultron setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        ...
        print_info("You can configure these later with 'ultron setup gateway'.")
