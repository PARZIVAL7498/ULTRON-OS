"""``ultron debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Ultron. Use 'ultron debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    ultron debug share              Upload debug report (asks for confirmation)
    ultron debug share --yes        Skip confirmation (for scripts/CI)
    ultron debug share --lines 500  Include more log lines
    ultron debug share --expire 30  Keep paste for 30 days
    ultron debug share --local      Print report locally (no upload)
    ultron debug share --no-redact  Disable upload-time secret redaction
    ultron debug share --nous       Upload to Nous-internal storage (private)
    ultron debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'ultron debug share'",
    )
