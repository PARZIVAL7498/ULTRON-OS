"""``ultron setup`` subcommand parser.
...
def build_setup_parser(subparsers, *, cmd_setup: Callable) -> None:
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Ultron with an interactive wizard. "
        "Run a specific section: "
        "ultron setup model|tts|terminal|gateway|tools|telemetry|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'ultron setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
