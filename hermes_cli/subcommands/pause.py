"""``ultron pause`` / ``ultron resume`` — the global emergency stop.

``ultron pause`` writes the ESTOP sentinel at ``$HERMES_HOME/ESTOP``, which
halts cron dispatch, kanban dispatch, and new gateway turns on their next
check. In-flight work is never killed. ``ultron resume`` removes the
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    print(
        ...
        "    In-flight work keeps running. Run `ultron resume` to lift the pause."
    )
    return 0
...
def build_pause_parser(subparsers) -> None:
    ...
    pause_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "`ultron resume`. In-flight work is never killed."
        ),
    )
    ...
    resume_parser = subparsers.add_parser(
        "resume",
        help="Lift the emergency stop set by `ultron pause`",
