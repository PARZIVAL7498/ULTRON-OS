"""``ultron peer`` — bot-to-bot DMs across machines/gateways.
...
bot on THIS machine a transport to message bots on THAT machine:
    ...
    ultron peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>
    ultron peer dm spark "Message from 🤖 dixie (@dixie): disk status?"
    ultron peer dm spark/researcher "..."      # named profile (multiplexed peer)
...
``ultron -p <bot> chat --in ~ -c "Bot Chat" ...`` bot-messaging command, so
...
def _parse_target(target: str) -> tuple[str, str | None]:
    ...
    if not peer:
        raise ValueError("Peer name required (ultron peer dm <peer>[/<agent>] ...)")
...
def cmd_peer(args) -> int:
    ...
    if action in ("add", "set"):
        ...
        if key:
            ...
            print(
                ...
                f"  ultron peer add {name} --url {url} --key <key>\n"
    ...
    if action in ("list", "ls", None):
        peers = _load_peers()
        if not peers:
            print("No peers registered. Add one: ultron peer add <name> --url http://host:port --key <API_SERVER_KEY>")
            return 0
    ...
    if action == "dm":
        ...
        if not isinstance(peer, dict) or not peer.get("url"):
            print(f"No peer named '{peer_name}'. Run: ultron peer list", file=sys.stderr)
            return 1
        key = _peer_secret(peer_name)
        if not key:
            print(
                f"No API key for peer '{peer_name}'. Set it: ultron peer add {peer_name} "
    ...
    print("Unknown peer action. See: ultron peer --help", file=sys.stderr)
    return 2
...
def build_peer_parser(subparsers) -> None:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            ...
            "'ultron peer dm <peer>[/<agent>] \"...\"' delivers into the remote "
            "agent's canonical Bot Chat over the peer's API server and prints "
            "the reply — the cross-machine twin of 'ultron -p <bot> chat'. "
        ...
        epilog=(
            ...
            "  ultron peer list\n"
            '  ultron peer dm spark "Message from 🤖 dixie (@dixie): disk status?"\n'
            '  ultron peer dm spark/researcher "..."   # named profile on a multiplexed peer\n'
            "  ultron peer remove spark\n"
            "\n"
