"""``ultron login`` subcommand parser.
...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``ultron login`` was removed in favor of ``ultron auth`` / ``ultron model``
    ...
    old scripts/aliases invoking ``ultron login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``ultron --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``ultron login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``ultron model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `ultron auth` to manage credentials, "
            "`ultron model` to select a provider, or `ultron setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `ultron model`",
    )
    login_parser.add_argument(
