"""``ultron gateway`` and ``ultron proxy`` subcommand parsers.
...
def build_gateway_parser(
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "Requires being logged in (ultron setup). Not available in managed installs."
        ),
    )
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `ultron proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
