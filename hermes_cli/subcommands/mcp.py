...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use 'ultron mcp add' to connect to a new server, or\n"
            "'ultron mcp serve' to expose Hermes conversations over MCP."
        ),
    )
    ...
    # flag is omitted, causing `ultron mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `ultron mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `ultron mcp install n8n`)",
    )
