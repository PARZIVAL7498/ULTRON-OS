"""``ultron sync`` subcommand parser — Skill Sync.
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    ultron sync status                 show gate/opt-in/head state
    ultron sync pull                   pull and materialize opted-in skills
    ultron sync push                   push opted-in skills
    ultron sync now                    reconcile: pull then push
    ultron sync enable <skill>         opt a skill into sync
    ultron sync disable <skill>        opt a skill out of sync
    ultron sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    ultron sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  ultron sync status            what is synced, and from where\n"
            "  ultron sync enable my-skill   include a skill in your sync\n"
            "  ultron sync now               pull, then push\n"
            "  ultron sync propose my-skill  share a skill with your team\n"
        ),
