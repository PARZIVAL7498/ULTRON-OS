"""``ultron update`` subcommand parser.
...
def build_update_parser(subparsers, *, cmd_update: Callable) -> None:
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Ultron to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'ultron config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--keep-stash",
        action="store_true",
    ...
    update_parser.add_argument(
        ...
        help=(
            ...
            "If the local checkout is on a different branch, ultron will "
