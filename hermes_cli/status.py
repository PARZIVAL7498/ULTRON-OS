...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``ultron config``'s
...
def _estop_status_line():
    """One-line pause banner for `ultron status`, or None when not paused.
...
def show_status(args):
    ...
    print(color("│                ⚕ Ultron Status                         │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `ultron status`
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: ultron model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: ultron auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: ultron auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: ultron model)"
    ...
    try:
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   ultron gateway")
    ...
    print(color("  Run 'ultron doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'ultron setup' to configure", Colors.DIM))
    print()
