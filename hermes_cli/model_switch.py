...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``ultron model`` wizard — not a
    ...
    while ``ultron model`` still live-probed the full ``/v1/models`` list.
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``ultron config set model.aliases.xxx``)
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'ultron model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'ultron doctor' — config issues detected:"
...
def list_authenticated_providers(
    ...
    *,
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # /model picker sees the SAME list `ultron model` would build, with
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # `ultron model` picker deliberately shows ONLY the curated agentic
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``ultron model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``ultron model``. Pin with
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for grp in groups.values():
            ...
            #   ``ultron model``. Pin a dict catalog with
