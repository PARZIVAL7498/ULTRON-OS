...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `ultron update` or `--replace` handoffs)
        ...
        message = (
            ...
            "with this token, then restart the gateway with 'ultron gateway restart'."
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``ultron update --gateway``
    ...
    def _build_provider_keyboard(self, providers: list, page: int = 0) -> tuple:
        ...
        CLI ``ultron model`` picker via the shared ``group_providers`` fold,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `ultron setup` to install Telegram support.",
