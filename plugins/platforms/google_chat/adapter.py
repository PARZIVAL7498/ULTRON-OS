...
# ``model_tools`` import time, so ``ultron status``, ``ultron chat``, etc.
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``ultron setup``.
    ...
    print_info("Restart the gateway: ultron gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``ultron cron`` running as a
    separate process from ``ultron gateway``).  Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `ultron setup` to install Google Chat support.",
