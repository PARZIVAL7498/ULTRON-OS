    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not check_whatsapp_requirements():
            ...
            self._set_fatal_error(
                ...
                "Node.js is not installed — install Node.js and re-run `ultron gateway`.",
                retryable=False,
            )
            return False
        ...
        try:
            ...
            # `ultron update` bumps the Baileys pin).  The stamp file records
                ...
                try:
                    ...
                    if install_result.returncode != 0:
                        ...
                        self._set_fatal_error(
                            ...
                            f"WhatsApp bridge npm install failed. Run `cd {bridge_dir} && {_npm_bin} install` manually, then restart `ultron gateway`.",
                            retryable=False,
                        )
                        return False
                    ...
                    self._set_fatal_error(
                        "whatsapp_npm_install_failed",
                        f"WhatsApp bridge npm install failed ({e}). Run `cd {bridge_dir} && {_npm_bin} install` manually, then restart `ultron gateway`.",
                        retryable=False,
                    )
                    return False
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            if bridge_status == "connected":
                                ...
                                # `ultron update`, so without this check it
...
def _is_connected(config) -> bool:
    ...
    would make WhatsApp always show as "configured" in ``ultron setup`` even
