...
Handles: ultron honcho setup | status | sessions | map | peer
"""
...
def cmd_sync(args) -> None:
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'ultron honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'ultron honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `ultron update` -- no output, no exceptions.
    """
    try:
        from hermes_cli.profiles import list_profiles
        profiles = list_profiles()
...
def cmd_setup(args) -> None:
    ...
    if not is_local:
        ...
        if use_device:
            ...
            try:
                ...
                print("\n  Cancelled. Re-run 'ultron honcho setup' to try again.\n")
                return
            except (AuthorizationTimeout, DeviceCodeExpired):
                print("\n  Device code expired before approval.")
                print("  Re-run 'ultron honcho setup' to get a new code.\n")
                return
            except AccessDenied:
                print("\n  Sign-in was denied on the approval page.")
                print("  Re-run 'ultron honcho setup' to retry, or choose an API key instead.\n")
                return
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'ultron honcho setup' again once you have a key.\n")
                return
    ...
    try:
        ...
        print("  Run: ultron config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    ultron honcho status     -- show full config")
    print("    ultron honcho mode       -- change recall/observation mode")
    print("    ultron honcho tokens     -- tune context and dialectic budgets")
    print("    ultron honcho peer       -- update peer names")
    print("    ultron honcho map <name> -- map this directory to a session name\n")
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: ultron honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'ultron honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'ultron honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: ultron honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    Hermes' identity in Honcho. Seed with 'ultron honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: ultron honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: ultron honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("\n  Set with: ultron honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'ultron honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'ultron honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    ultron honcho identity --show        — show both peer representations")
        print("    ultron honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    ...
    if has_key:
        ...
        print("  2. Run:  ultron honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'ultron honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'ultron honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  ultron honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        for f in user_files:
            print("    ultron honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'ultron honcho setup' first, then re-run this step.")
    else:
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'ultron honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    ultron honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  ultron honcho identity <path/to/SOUL.md>")
    ...
    print("              Map a custom name:  ultron honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. ultron honcho setup              — configure API key (required)")
        print("  2. ultron honcho migrate            — re-run this walkthrough")
    else:
        print("  1. ultron honcho status             — verify Honcho connection")
        print("  2. hermes                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. ultron honcho identity --show    — verify AI peer representation")
        print("  4. ultron honcho tokens             — tune context and dialectic budgets")
        print("  5. ultron honcho mode               — view or change memory mode")
    print()
...
def register_cli(subparser) -> None:
    """Build the ``ultron honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``ultron honcho``.
    """
