...
def _credential_fingerprint(config: HonchoClientConfig | None) -> str:
    ...
    makes 'ultron honcho setup' account switches produce a NEW cache identity
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'ultron honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `ultron honcho setup` to configure)."
            )
