...
models show up in ``ultron tools`` automatically. No model ids are
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "DEEPINFRA_API_KEY not set. Run `ultron tools` → Image "
                    "Generation → DeepInfra to configure, or `ultron setup` "
                    "to add the key."
                ),
