...
# image_gen.model from ``ultron tools``): use exactly that model (no auto
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        after this code shipped — is selectable in ``ultron tools``. Nous
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``ultron tools``) → the quality-first default chain.
        ...
        if not api_key or not base_url:
            return error_response(
                error=(
                    ...
                    f"Configure {self._display} in `ultron tools` → Image Generation."
                ),
                error_type="missing_api_key",
                provider=self._name,
                aspect_ratio=aspect_ratio,
