...
three virtual model IDs so the ``ultron tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `ultron tools` → Image "
                    "Generation → OpenAI to configure, or `ultron setup` "
                    "to add the key."
                ),
