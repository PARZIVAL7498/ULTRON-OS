...
def init_agent(
                ...
                if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                    ...
                    if not _fb_resolved:
                        raise RuntimeError(
                            ...
                            f"variable, or switch to a different provider with `ultron model`."
                        )
                if not getattr(agent, "_fallback_activated", False):
                    # No provider configured — reject with a clear message.
                    raise RuntimeError(
                        "No LLM provider configured. Run `ultron model` to "
                        "select a provider, or run `ultron setup` for first-time "
                        "configuration."
                    )
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Ultron.  Choose a model with at least "
